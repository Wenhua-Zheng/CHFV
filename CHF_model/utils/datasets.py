...
def create_dataloader(path, labels_path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
                      rank=-1, world_size=1, workers=8, image_weights=False, quad=False, prefix=''):
    # Make sure only the first process in DDP process the dataset first, and the following others can use the cache
    with torch_distributed_zero_first(rank):
        dataset = LoadImagesAndLabels(path, labels_path, imgsz, batch_size,
...
def img2label_paths(img_paths, labels_path, data_type):
    # Define label paths as a function of image paths
    test_labels_path = []
    for x in tqdm(img_paths, desc="img_paths -> label_paths"):
        if "train" in data_type:
            test_labels_path.append(os.path.join(labels_path, "train\\labels", os.path.basename(x)))
        elif "val" in data_type:
            test_labels_path.append(os.path.join(labels_path, "val\\labels", os.path.basename(x)))
        elif "test" in data_type:
            test_labels_path.append(os.path.join(labels_path, "test\\labels", os.path.basename(x)))
    return [x.replace(x.split('.')[-1], 'txt') for x in test_labels_path]


class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, labels_path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        # Check cache
        self.label_files = img2label_paths(self.img_files, self.labels_path, path)  # labels
        ...
        self.label_files = img2label_paths(cache.keys(), self.labels_path, path)  # update
        if single_cls:
